...
function getLesCovoitUser(){
	...
	$requete=$pdo->prepare('SELECT id, nom, prenom, tel, mail, mdp FROM CovoitUser');
        $requete->execute();
...
function getUnCovoitUser($id){
	$pdo = PDO2::getInstance();
 	$requete=$pdo->prepare('SELECT id, nom, prenom, tel, mail, mdp FROM CovoitUser WHERE id = :id');
...
function addCovoitUser($unCovoitUser){
	   ...
	   $bv5 = $requete->bindValue(':mdp', $unCovoitUser->getMdp());
	   $requete->execute();
